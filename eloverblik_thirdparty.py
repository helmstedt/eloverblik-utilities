# A utility to interact with the api from eloverblik.dk as a third party
...
# Number of API retries (API sometimes returns 503 errors)
api_retries = 10

# API base url
base_url = 'https://api.eloverblik.dk/ThirdPartyApi/api/'
...
def get_authorizations():
    print('Getting list of authorizations...')
    get_authorizations = get_endpoint('authorization/authorizations')
    print(f'Found {len(get_authorizations["result"])} authorizations(s)')
    print('Printing list of authorizations(s)...\n')
    for authorization in get_authorizations['result']:
        authorization_count = 1
        print(f'--- Authorization {authorization_count} ---')
        for key, value in authorization.items():
            print(key, ':', value)
        print('\nPrinting metering points for authorization...')
        get_metering_points = get_endpoint(f"authorization/authorization/meteringpoints/customerKey/{authorization['customerKey']}")
        meter_count = 1
        for metering_point in get_metering_points['result']:
            print(f'\n--- Authorization {authorization_count}: Meter {meter_count} ---')
            for key, value in metering_point.items():
                if key == 'childMeteringPoints':
                    if value:
                        print('--- Child meters ---')
                        child_meter_count = 1
                        print(f'--- Child meter {child_meter_count} ---')
                        for child_meter in value:
                            for k, v in child_meter.items():
                                print(k, ':', v)
                            child_meter_count += 1  
                    else:
                        print('Meter has no child meters')
                else:
                    print(key, ':', value)            
            meter_count +=1
        print(f'--- End of authorization {authorization_count} ---\n')
        authorization_count += 1
    sys.exit('All authorizations and meters printed. Exiting.')
...
def get_charges_data(meter_ids):
    ...
    with open('eloverblik_charges_data.csv', 'w', newline='') as csvfile:
        ...
        for meter_id in meter_ids:
            ...
            charges_data_endpoint = 'meteringpoint/getcharges'
...
def main():
    ...
    parser.add_argument('-m', '--mode', help='Mode: List authorizations with metering points or get data.', type=str, choices=['list', 'get'], required=True)
    parser.add_argument('-n', '--meterid', help='Get data from this specific meter. Required in get mode.', type=str)
    ...
    # If mode is list authorizations, get a list of authorizations
    if args.mode == 'list':
    ...
    # If mode is get data, get data
        ...
        # Prepare meter_ids list and validate argument
        if args.meterid:
            meter_ids = [args.meterid]
        else:
            sys.exit('Error: You must specify a meter id in get mode. Exiting.')
        ...
        # Get data from meter
        print('Getting and saving usage and charges data for meter...')
        # Get usage data
        get_usage_data(meter_ids, args, periods)
        # Get charges data
        get_charges_data(meter_ids)
        # Print status
        print('Saved usage and charges data for meter')
